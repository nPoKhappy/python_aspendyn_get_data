...
def _is_comtypes_dynamic_object(value):
    value_type = type(value)
    return value_type.__module__ == "comtypes.client.dynamic"


def _wrap_com_object(value):
    if isinstance(value, AspenComObject):
        return value
    if _is_comtypes_dynamic_object(value):
        return AspenComObject(value)
    return value


def _unwrap_com_object(value):
    if isinstance(value, AspenComObject):
        return value._com_object
    return value


class AspenComObject:
    """Small compatibility layer so existing win32com-style calls keep working."""
    ...
    def __getattr__(self, name):
        try:
            return _wrap_com_object(getattr(self._com_object, name))
        except comtypes.COMError as exc:
            details = exc.args[2] if len(exc.args) > 2 else None
            message = details[0] if details else ""
            if message == "Mismatch in number of array indices":
                return AspenIndexedMember(self._com_object, name)
            raise

    def __setattr__(self, name, value):
        setattr(self._com_object, name, _unwrap_com_object(value))

    def __call__(self, *args):
        item_error = None
        try:
            item = getattr(self._com_object, "Item", None)
        except (TypeError, ValueError, comtypes.COMError, OSError) as exc:
            item = None
            item_error = exc

        if item is not None:
            try:
                return _wrap_com_object(item(*args))
            except (TypeError, ValueError, comtypes.COMError, OSError):
                pass

        if callable(self._com_object):
            return _wrap_com_object(self._com_object(*args))

        if hasattr(self._com_object, "_comobj"):
            try:
                return _wrap_com_object(self._com_object._comobj.Invoke(0, *args))
            except (TypeError, ValueError, comtypes.COMError, OSError):
                pass

        if item_error is not None:
            raise item_error
        raise TypeError(f"{type(self._com_object).__name__!r} object is not callable")

    def __repr__(self):
        return repr(self._com_object)


class AspenIndexedMember:
    def __init__(self, com_object, name):
        self._com_object = com_object
        self._name = name
        self._dispid = com_object._comobj.GetIDsOfNames(name)[0]

    def __call__(self, *args):
        return _wrap_com_object(self._com_object._comobj.Invoke(self._dispid, *args))


class Env(object):
    def __init__(self, dt, MAX_EP_STEPS, aspen_file_name):
        # dt:每一步的模擬時間
        # max_ep_steps: 每個訓練及的最大步數

        ##############  Aspen啟用  ######################################
        project_dir = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
        if os.path.isabs(aspen_file_name):
            aspen_file_path = aspen_file_name
        else:
            aspen_file_path = os.path.join(project_dir, "ASPen_file", aspen_file_name)
        ad_path = r"C:\Program Files\AspenTech\AMSystem V14.0\Bin\AspenModeler.exe"
        ad_working_dir = r"C:\ProgramData\AspenTech\Aspen Plus Dynamics V14.0"
        prog_id = "AD Application"

        if not os.path.exists(aspen_file_path):
            raise FileNotFoundError(f"Aspen Dynamics 檔案不存在: {aspen_file_path}")
        if not os.path.exists(ad_path):
            raise FileNotFoundError(f"AspenModeler.exe 不存在: {ad_path}")
        if not os.path.isdir(ad_working_dir):
            raise FileNotFoundError(f"Aspen Dynamics 工作目錄不存在: {ad_working_dir}")
