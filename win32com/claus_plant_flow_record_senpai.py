...
#讀資料
    def get_input_composition(self): #獲取進料資訊
        ...
        acidgas_Fv = self.streams('ACIDGAS').Fv.value    # 體積流率 m³/h（Aspen 依 T、P 自動計算）
        ...
        return acidgas_Fm, acidgas_Fv, acidgas_CO2, acidgas_H2O, acidgas_H2S, acidgas_T, acidgas_P, air, air_SP
    ...
    def data_conclusion(self):
        ...
        data0, data79, data1, data2, data3, data4, data5, data6, data73 = self.get_input_composition()
        data0 = np.array([data0])
        data79 = np.array([data79])  # acidgas_Fv (m³/h)
        ...
        print("conv=",data76)
        return np.concatenate([data0, data79, data1, data2, data3, data4, data5, data6, data73, data7, data74, data8, data75, data9, data10, data11, data12, data13, data14, data15, data16, data17, data18, data19, data20, data21, data22, data23, data24, data25, data26, data27, data28, data29, data30, data31, data32, data33, data34, data35, data36, data37, data38, data39, data40, data41, data42, data43, data44, data45, data46, data47, data48, data49, data50, data51, data52, data53, data54, data55, data56, data57, data58, data59, data60, data61, data62, data63, data64, data65, data66, data67, data68, data69, data76])
    ...
    def disturbance(self, ram):        #gauss random disturbance
        ...
        # 以工廠體積流量統計生成 Fv，再用即時 T、P 以理想氣體換算 kmol/h
        # plant: Fv mean=3002.671 m³/h, std=589.076 m³/h
        # component fractions: CO2=28.57%, H2O=32.32%, H2S=39.11%
        # rho = P / (R * T_K),  R = 0.08314 bar·m³/(kmol·K)
        Fv_total = max(min(random.gauss(3002.671, 589.076), 3002.671 + 2*589.076), 3002.671 - 2*589.076)  # m³/h, clip ±2σ → [1824.5, 4181.8]
        inlet_T  = max(min(random.gauss(82.75, 4.550),   82.75  + 2*4.550),  82.75  - 2*4.550)           # °C,   clip ±2σ → [73.65, 91.85]
        inlet_P  = max(min(random.gauss(1.1881, 0.0149), 1.1881 + 2*0.0149), 1.16)                      # bar,  clip upper ±2σ=1.218, hard lower=1.16
        rho = inlet_P / (0.08314 * (inlet_T + 273.15))   # kmol/m³ (理想氣體)
        F_total = Fv_total * rho                          # kmol/h
        Fn_carbon_dioxide  = max(F_total * 0.2857 + random.gauss(0, 1.023), 0.1)   # kmol/h, 獨立組成擾動
        Fn_hydrogen_dioxide = max(F_total * 0.3232 + random.gauss(0, 1.158), 0.1)  # kmol/h
        Fn_hydrogen_sulfide = max(F_total * 0.3911 + random.gauss(0, 1.380), 0.1)  # kmol/h
        print(f"Fv_total={Fv_total:.2f} m³/h, rho={rho:.5f} kmol/m³, F_total={F_total:.2f} kmol/h")
    ...
    def do_dis3(self, A,B,C, inlet_T, inlet_P, TR1, TR2, air2):
        ...
        # TR1 (B21) is not a manipulated variable; leave it at whatever the simulation has
    ...
    def disturbance_air2_T(self, steps): #gauss random disturbance
        ...
        # TR1 is not manipulated; always read from simulation
    ...
    def op(self, manual_time):
        op_TR2 = [random.gauss(193.36, 5.816) for _ in range(int(60 * 24 / manual_time))]  # plant mean=193.3593, std=5.815801
        op_air2 = [random.gauss(837.43, 130.51) for _ in range(int(60 * 24 / manual_time))]  # mean shifted +50 from plant mean=837.4348, std=130.5129
        print('op_TR2=', op_TR2)
