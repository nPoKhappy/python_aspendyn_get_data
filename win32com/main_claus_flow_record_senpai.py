...
import claus_plant_flow_record_senpai as plant  #環境-(Aspen)
import numpy as np
import pandas as pd
...
import random
###############################  Model setting  ####################################
PROJECT_NAME = 'plant_data'
OUTPUT_FILE = 'plant_data'
dt = 1  #每一步的模擬時間(MIN)
MAX_EP_STEPS = 1440 #每個訓練及的最大步數 (24h * 60min)
MAX_EPISODES = 5
...
def save_result(data, datacomp, episode, steps):
    ...
    data[(episode) * MAX_EP_STEPS + steps, 3:78] = env.data_conclusion()  # 第二欄開始
